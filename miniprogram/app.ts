...
const getTempIds = async (self: any) => {
  // 调用 wx.getSetting 获取用户的订阅模板 ID
  // itemSettings 只返回用户勾选过订阅面板中的“总是保持以上选择，不再询问”的订阅消息
  const res = await wx.getSetting({
    withSubscriptions: true,
  });
  self.globalData.subscriptionSetting = res.subscriptionsSetting;
  // 该项目的所有订阅消息模板 ID
  let arr: string[];
  try {
  ...
  const { mainSwitch, itemSettings } = self.globalData.subscriptionSetting;
  // mainSwitch 订阅消息总开关，true为开启，false为关闭
  if (!mainSwitch) return;
  // 如果没有代表用户未订阅任何消息
  if (!itemSettings) {
    self.globalData.tmplIds = arr;
  } else {
    // 用户已经有订阅的消息了，那么就找到用户还没订阅的模板ID
    const keysArr = Object.keys(itemSettings);
    self.globalData.tmplIds = arr.filter((item) => !keysArr.includes(item)); // 返回用户没有订阅的模板ID
  }
};
const initWs = async (cb: any) => {
  const ws = new Ws({}, cb);
  ws.connet();
};
App<IAppOption>({
  ...
  // 获取用户的模板订阅信息
  async onLaunch() {
    await getTempIds(this);
    await initWs((msg: any) => {
      this.globalData.msgCount = msg.message;
      if(msg.message !== 0 ){
        wx.setTabBarBadge({
          index: 1,
